...
export function HolidayTooltip({
	...
	const isOptional =
		holiday.provinces &&
		holiday.provinces.some((p) => p.nameEn && p.nameEn.includes('Optional'))
	...
	return (
		...
		>
			...
			{isVisible && (
				<div
					className='absolute z-50 p-3 bg-white dark:bg-gray-800 border border-purple-200 dark:border-purple-700 rounded-lg shadow-xl shadow-purple-500/20 min-w-[200px] max-w-[300px] bottom-full left-1/2 transform -translate-x-1/2 mb-2'
					onMouseEnter={() => setIsVisible(true)}
					onMouseLeave={() => setIsVisible(false)}
				>
					{/* Arrow */}
						...
						<div className='text-xs space-y-1'>
							...
							{holiday.observedDate !== holiday.date && (
								<div className='flex items-center gap-2'>
									...
									<span className='text-gray-900 dark:text-gray-100 font-medium'>
										{new Date(holiday.observedDate).toLocaleDateString(
											'en-CA',
											{
												weekday: 'long',
												year: 'numeric',
